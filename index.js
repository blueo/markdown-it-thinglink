...
function video_embed(md) {
    function video_return(state, silent) {
        ...
        var EMBED_REGEX = /@\[(youtube|vimeo|html5)\]\([\s]*(.*?)[\s]*[\)]/im;
...
function tokenize_html5(videoID) {
    var embedStart = '<video src="';
    var embedEnd = '" controls/>';
    return embedStart + videoID + embedEnd;
}
...
function tokenize_video(md) {
    function tokenize_return(tokens, idx, options, env, self) {
        ...
        } else if (service.toLowerCase() === 'html5') {
            return tokenize_html5(videoID);
        } else{
            return('');
        }
